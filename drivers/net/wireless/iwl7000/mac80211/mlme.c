...
#define IEEE80211_NEG_TTLM_REQ_TIMEOUT (CPTCFG_IWL_TIMEOUT_FACTOR * HZ / 5)
	...
	*chandef = vht_chandef;
	...
	if (eht_oper->params & IEEE80211_EHT_OPER_INFO_PRESENT) {
		...
		if (ieee80211_eht_oper_dis_subchan_bitmap(eht_oper)) {
			sdata_info(sdata,
				   "puncturing not supported, disabling EHT\n");
			return IEEE80211_CONN_MODE_HE;
		}
				     ...
				     u32 prohibited_flags)
{
	...
	if (0 && chandef->chan->band == NL80211_BAND_5GHZ &&
					...
					const u16 *outer_present_elems)
{
	...
	if (ift_ext_capa) {
		eml_capa = cpu_to_le16(0);
		mld_capa_ops = cpu_to_le16(0);
	}
...
static void __ieee80211_disconnect_unlocked(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
	...
	if (elems->he_operation &&
		...
		*changed |= ieee80211_recalc_twt_req(sdata, sband, link,
		...
		if (elems->eht_operation && elems->eht_cap &&
			...
			bss_conf->eht_support = false/* no EHT */;
		} else {
...
static void _ieee80211_sta_rx_queued_mgmt(struct ieee80211_sub_if_data *sdata,
				  struct sk_buff *skb)
{
	struct ieee80211_link_data *link = &sdata->deflink;
	struct ieee80211_rx_status *rx_status;
...
static void _ieee80211_sta_work(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
...
#ifdef CONFIG_PM
static void _ieee80211_mgd_quiesce(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
	u8 frame_buf[IEEE80211_DEAUTH_FRAME_LEN];
...
static void _ieee80211_sta_restart(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
		       ...
		       struct cfg80211_auth_request *req)
{
	...
	memcpy(auth_data->ap_addr,
	       NULL/* no MLD support */ ?: req->bss->bssid,
	       ETH_ALEN);
			...
			struct cfg80211_assoc_request *req)
{
	...
	err = ieee80211_prep_connection(sdata, cbss, -1,
					NULL/* no MLD support */, true,
			   ...
			   struct cfg80211_disassoc_request *req)
{
	...
	if (!sdata->u.mgd.associated ||
	    memcmp(sdata->vif.cfg.ap_addr, req->bss->bssid, ETH_ALEN))
		return -ENOTCONN;
